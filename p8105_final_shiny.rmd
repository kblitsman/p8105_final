...
```{r create_scatter}
# This scatter graph presents a visual interpretation for all fifty states and the estimated total population that uses a specific substance relative to the year selected. States with greater populations will be located farther on the right side of the graph and substances with greater estimated users will be located higher on the graph. This should provide users with a method of comparing states to one another relative to their population size.   

renderPlotly({
saes %>%
  filter(agegrp == input$age_group, outname == input$outcome_choice, pyearnm == input$year_range) %>%
  plot_ly(
