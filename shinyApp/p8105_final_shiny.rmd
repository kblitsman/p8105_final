...
renderPlotly({
  saes %>% 
    ...
    plot_ly(x = ~pyearnm, y = ~est_total, type = "scatter", mode = "lines+markers", hoverinfo = 'text', text = ~paste("Years:", pyearnm, '<br>', "Total count:", est_total),
          marker = list(size = 10,
...
p = saes %>%
  ...
  ggplot(aes(x = pyearnm, y = est_total*1000/pop*100, fill = stname)) +
...
# Attempt to make hovertext informative. Plotly doe snot handle hovertext well when creating the graph from ggplot
